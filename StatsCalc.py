...
def hypothesisTwoPopulationsF(o1, o2, n1, n2, alpha, tail):
    ...
    if (tail == "two"):
        ...
        printLatex("F_1 = F(" + str(alpha/2) + ", " + str(df1) +
                   ", " + str(df2) + ") = " + str(criticalF1))

        criticalF2 = fTest(1-alpha/2, df1, df2)
        printLatex("F_2 = F(" + str(1-alpha/2) + ", " + str(df1) +
                   ", " + str(df2) + ") = " + str(criticalF2))
        if (f > criticalF1 and f < criticalF2):
            printLatex("f \\in (" + str(criticalF1) + ", " +
                       str(criticalF2) + ") Accept H_0")
        else:
            printLatex("f \\notin (" + str(criticalF1) +
                       ", " + str(criticalF2) + ") Reject H_0")
    if (tail == "left"):
        criticalF = fTest(alpha, df1, df2)
        printLatex("F = F(" + str(alpha) + ", " + str(df1) +
                   ", " + str(df2) + ") = " + str(criticalF))
        if (f < criticalF):
    ...
    if (tail == "right"):
        ...
        printLatex("F = F(" + str(1-alpha) + ", " + str(df1) +
                   ", " + str(df2) + ") = " + str(criticalF))
        if (f > criticalF):
            printLatex("f > " + str(criticalF) + " Accept H_0")
        else:
...
def hypothesisTwoPopulationsPaired(dbar, sd, n, alpha, tail):
    ...
    if (tail == "two"):
        ...
        printLatex("t = t(" + str(1-alpha/2) + ", " +
                   str(df) + ") = " + str(criticalT))
        if (t > criticalT):
...
def hypothesisTestingCase1(x, mu, std, pop, alpha, tail):
    ...
    printLatex("z = \\frac{" + str(x) + " - " + str(mu) + "}{" +
               str(std) + " / \\sqrt{" + str(pop) + "}} = " + str(z))
...
def hypothesisTestingPopulationPortion(population, hypothesized, positive, alpha, tail):
    ...
    printLatex("\\hat{p} = \\frac{" + str(positive) +
               "}{" + str(population) + "} = " + str(phat))
    ...
    printLatex(
        "z = \\frac{\\hat{p} - p_0}{\\sqrt{\\frac{p_0 q_0}{n}}} = " + str(z))
...
def hypothesisTwoPopulationsZ(x1, x2, s1, s2, n1, n2, alpha, tail):
    ...
    printLatex("z = \\frac{{x_1 - x_2}}{{\\sqrt{{\\frac{{s_1^2}}{{n_1}} + \\frac{{s_2^2}}{{n_2}}}}}} = \\frac{{{}}}{{{}}} = {}".format(
        x1-x2, math.sqrt(s1**2/n1 + s2**2/n2), z))
    ...
    if (tail == "two"):
        ...
        if (z < zTable(alpha/2, "left")):
            ...
            printLatex(
                "z < z_{{\\alpha/2}} = {} \\Rightarrow \\text{{Fail to reject }} H_0".format(c))
        ...
        if (z < zTable(alpha, "left")):
            ...
            printLatex(
                "z < z_{{\\alpha}} = {} \\Rightarrow \\text{{Fail to reject }} H_0".format(c))
...
def hypothesisTwoPopulationsT(x1, x2, s1, s2, n1, n2, alpha, tail):
    ...
    printLatex("t = \\frac{{x_1 - x_2}}{{\\sqrt{{\\frac{{s_1^2}}{{n_1}} + \\frac{{s_2^2}}{{n_2}}}}}} = \\frac{{{}}}{{{}}} = {}".format(
        x1-x2, math.sqrt(s1**2/n1 + s2**2/n2), t))
    ...
    if (equal == "y"):
        ...
        printLatex(
            "sp^2 = \\frac{{(n_1-1)s_1^2 + (n_2-1)s_2^2}}{{n_1+n_2-2}} = \\frac{{({}-1){}^2 + ({}-1){}^2}}{{{}+{}-2}} = {}".format(n1, s1, n2, s2, n1, n2, sp2))
        # t statistic
        sp = sp2**(1/2)
        printLatex("sp = \\sqrt{{sp^2}} = \\sqrt{{{}}} = {}".format(sp2, sp))
        # print sp
        t = (x1-x2) / (sp2*(1/n1 + 1/n2))**(1/2)
        printLatex("t = \\frac{{x_1 - x_2}}{{sp^2\\sqrt{{\\frac{{1}}{{n_1}} + \\frac{{1}}{{n_2}}}}}} = \\frac{{{}}}{{{}}} = {}".format(
            x1-x2, sp2*(1/n1 + 1/n2)**(1/2), t))
    ...
    if (tail == "two"):
        if (t > c):
            reject = True
            printLatex(
                "t > t_{%d, %.4f} = %.02f \\Rightarrow \\text{Reject } H_0" % (df, alpha, c))
        elif (t < -c):
            c = -c
            reject = True
            printLatex(
            ...
            printLatex("t < t_{%d, %.4f} = %.02f \\Rightarrow \\text{Fail to reject } H_0" % (
                df, alpha, c))
    if (tail == "left"):
        if (t < -c):
            c = -c
            ...
            printLatex(
                "t > -t_{%d, %.4f} = %.02f \\Rightarrow \\text{Fail to reject } H_0" % (df, alpha, c))
    if (tail == "right"):
        if (t > c):
            reject = True
            printLatex(
                "t > t_{%d, %.4f} = %.4f \\Rightarrow \\text{Reject } H_0" % (df, alpha, c))
        else:
            printLatex("t < t_{%d, %.4f} = %.4f \\Rightarrow \\text{Fail to reject } H_0" % (
                df, alpha, c))
    return t, c, reject
...
def hypothesisTwoPopulationsT2(x1, x2, s1, s2, n1, n2, alpha, tail):
    ...
    if (tail == "two"):
        ...
        if (t > criticalT):
            printLatex("t > t_{%d, %.4f} = %.4f \\Rightarrow \\text{Reject } H_0" % (
                v, alpha/2, criticalT))
            reject = True
        elif (t < -criticalT):
            printLatex(
                "t < -t_{%d, %.4f} = %.4f \\Rightarrow \\text{Reject } H_0" % (v, alpha/2, criticalT))
            reject = True
        else:
            printLatex("t < t_{%d, %.4f} = %.4f \\Rightarrow \\text{Fail to reject } H_0" % (
                v, alpha/2, criticalT))
    ...
    if (tail == "left"):
        ...
        if (t < -criticalT):
            printLatex(
                "t < -t_{%d, %.4f} = %.4f \\Rightarrow \\text{Reject } H_0" % (v, alpha, criticalT))
            reject = True
        else:
            printLatex(
                "t > -t_{%d, %.4f} = %.4f \\Rightarrow \\text{Fail to reject } H_0" % (v, alpha, criticalT))
            reject = False
    if (tail == "right"):
        printLatex("t_{%d, %.4f} = %.4f" % (v, alpha, criticalT))
        if (t > criticalT):
...
def hypothesisTestingTwoProportions(x1, x2, n1, n2, alpha, tail):
    ...
    phat = (x1+x2)/(n1+n2)
    printLatex("\\hat{p} = \\frac{x_1 + x_2}{n_1 + n_2} = \\frac{%d + %d}{%d + %d} = %.4f" %
               (x1, x2, n1, n2, phat))
    ...
    if (tail == "two"):
        if (z > zTable(alpha/2)):
            printLatex(
                "z > z_{{%.4f/2}} = %.4f \\Rightarrow \\text{Reject } H_0" % (alpha, zTable(alpha/2)))
            reject = True
            ...
            printLatex(
                "z < z_{{%.4f/2}} = %.4f \\Rightarrow \\text{Fail to Reject } H_0" % (alpha, zTable(alpha/2)))
    if (tail == "left"):
        if (z < -zTable(alpha)):
            printLatex(
                "z < -z_{{%.4f}} = %.4f \\Rightarrow \\text{Reject } H_0" % (alpha, zTable(alpha)))
            reject = True
        else:
            printLatex(
                "z > -z_{{%.4f}} = %.4f \\Rightarrow \\text{Fail to reject } H_0" % (alpha, zTable(alpha)))
    if (tail == "right"):
        if (z > zTable(alpha)):
            printLatex("z > z_{{%.4f}} = %.4f \\Rightarrow \\text{Reject } H_0" % (
                alpha, zTable(alpha)))
            reject = True
        else:
            printLatex("z < z_{{%.4f}} = %.4f \\Rightarrow \\text{Fail to reject } H_0" % (
                alpha, zTable(alpha)))
...
def hypothesisTwoMeans():
    ...
    printLatex("x_1 = %.4f, x_2 = %.4f, s_1 = %.4f, s_2 = %.4f, n_1 = %d, n_2 = %d, \\alpha = %.4f, tail = %s" % (
        x1, x2, s1, s2, n1, n2, alpha, tail))

    # calculate t-statistic and p-value
    t, c, reject = hypothesisTwoPopulations(
        x1, x2, s1, s2, n1, n2, alpha, tail)
...
def hypothesisTwoProportions():
    ...
    # print all data in one line
    printLatex("x_1 = %d, x_2 = %d, n_1 = %d, n_2 = %d, \\alpha = %.4f, tail = %s" % (
        x1, x2, n1, n2, alpha, tail))
...
def hypothesisOneMean():
    ...
    printLatex("x = %.4f, \\mu = %.4f, s = %.4f, n = %d, \\alpha = %.4f, tail = %s" % (
        x, mu, s, n, alpha, tail))
...
def hypothesisOneProportion():
    ...
    # print all data in one line, space separated
    printLatex("x = %.4f, \\mu = %.4f, n = %d, \\alpha = %.4f, tail = %s" %
               (sample, hyp, n, alpha, tail))
...
if __name__ == "__main__":
    ...
    if (choice == 1):
        ...
        result = hypothesisTwoPopulationsF(
            float(input("variance of sample 1: ")), float(
                input("variance of sample 2: ")),
